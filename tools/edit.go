...
// artifacts storage directory is always editable regardless of
// allowed_roots.
func (t *EditTool) validatePath(p string) error {
	...
	if len(t.cfg.AllowedRoots) == 0 {
		return fmt.Errorf("path %q is denied: no allowed_roots configured; set spec.config.tools.edit.allowed_roots to permit access", p)
	}
